        ...
        this.raw_data = col_cfgs.map((col) => {			
            ...
            if ("attr" in col) {
                return ((col.attr in this.device.attributes) ?
                    this.device.attributes[col.attr] : null);
                ...
                // handle device name customization
                    ...
                    if ("user_given_name" in this.device.attributes && this.device.attributes["user_given_name"])
...
class ZHANetworkCard extends HTMLElement {
    ...
    _updateContent(element, rows) {
        ...
        element.innerHTML = rows.map((row) => 
            `<tr id="device_row_${row.device.attributes.device_reg_id}">${row.data.map(
                (cell) => ((!cell.hide) ? 
        ...
        rows.forEach(row => {
            const elem = this.shadowRoot.getElementById(`device_row_${row.device.attributes.device_reg_id}`);
